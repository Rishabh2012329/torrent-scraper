...
export class Scraper {
  ...
  async fetchMovies(startUrl, movieName) {
    ...
    startUrl += `/search/keywords:${movieName}:category:201/`;
    ...
    try {
      ...
      let data = await page.evaluate(() => {
        ...
        let items = document.querySelectorAll(
          "span.list-item.item-name.item-title"
        );
      ...
      let p = await data.map((obj, index) => {
        return new Promise(async (resolve, reject) => {
          data[index] = {
            ...data[index],
            magnetUrl: await this.getMagnetUrl(obj.link, browser),
          };
          resolve();
        });
      });
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (let i = 0; i < proxylist.length; i++) {
          ...
          if (movies.length > 5) data = [...data, ...movies];
  ...
  createCSV() {
    ...
    for (let q in this.questions) {
      data += `'${this.questions[q].questionLink}',${this.questions[
        q
      ].title?.replace(",", " ")},${this.questions[q].answerCount},${
        this.questions[q].upvotes
      },${this.questions[q].count}\n`;
    }
